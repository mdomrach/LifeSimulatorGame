...
void FEnvironment::DestroyBuffers(FVulkanDevice vulkanDevice)
{
	...
	terrainIndexBuffer.Destroy(vulkanDevice.logicalDevice);
	terrainVertexBuffer.Destroy(vulkanDevice.logicalDevice);
...
void FEnvironment::PreparePipeline(VkDevice logicalDevice, VkGraphicsPipelineCreateInfo* pipelineInfo)
{
	...
	auto vertShaderCode = FFileCalculator::ReadFile("shaders/vert.spv");
	auto fragShaderCode = FFileCalculator::ReadFile("shaders/frag.spv");
...
void FEnvironment::BuildCommandBuffers(VkCommandBuffer commandBuffer, FScene* scene, VkPipelineLayout pipelineLayout)
{
	...
	//vkCmdBindVertexBuffers(commandBuffer, 0, 1, &terrainVertexBuffer.buffer, offsets);
	vkCmdBindVertexBuffers(commandBuffer, 0, 1, &vertexBuffer.buffer, offsets);
	//vkCmdBindIndexBuffer(commandBuffer, terrainIndexBuffer.buffer, 0, VK_INDEX_TYPE_UINT32);
	vkCmdBindIndexBuffer(commandBuffer, indexBuffer.buffer, 0, VK_INDEX_TYPE_UINT32);
	vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline);
	vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSet, 0, nullptr);
	//vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(scene->terrain->indices.size()), 1, 0, 0, 0);
...
void FEnvironment::CreateBuffers(FScene* scene, FVulkanDevice vulkanDevice, VkCommandPool commandPool, VkQueue graphicsQueue)
{
	...
	CreateTerrainVertexBuffer(scene, vulkanDevice, commandPool, graphicsQueue);
	CreateTerrainIndexBuffer(scene, vulkanDevice, commandPool, graphicsQueue);
...
void FEnvironment::CreateTerrainVertexBuffer(FScene* scene, FVulkanDevice vulkanDevice, VkCommandPool commandPool, VkQueue graphicsQueue)
{
	FMesh* terrain = scene->terrain;
	...
	VkBufferUsageFlags vertexBufferUsageFlags = VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
	VkMemoryPropertyFlags vertexMemoryPropertyFlags = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
	FVulkanBufferCalculator::CreateBuffer(vulkanDevice, bufferSize, vertexBufferUsageFlags, vertexMemoryPropertyFlags, terrainVertexBuffer.buffer, terrainVertexBuffer.bufferMemory);

	FVulkanBufferCalculator::CopyBuffer(vulkanDevice.logicalDevice, commandPool, graphicsQueue, stagingBuffer.buffer, terrainVertexBuffer.buffer, bufferSize);

	stagingBuffer.Destroy(vulkanDevice.logicalDevice);
}

void FEnvironment::CreateTerrainIndexBuffer(FScene* scene, FVulkanDevice vulkanDevice, VkCommandPool commandPool, VkQueue graphicsQueue)
{
	FMesh* terrain = scene->terrain;
	...
	VkBufferUsageFlags indexBufferUsageFlags = VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT;
	VkMemoryPropertyFlags indexMemoryPropertyFlags = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
	FVulkanBufferCalculator::CreateBuffer(vulkanDevice, bufferSize, indexBufferUsageFlags, indexMemoryPropertyFlags, terrainIndexBuffer.buffer, terrainIndexBuffer.bufferMemory);

	FVulkanBufferCalculator::CopyBuffer(vulkanDevice.logicalDevice, commandPool, graphicsQueue, stagingBuffer.buffer, terrainIndexBuffer.buffer, bufferSize);
